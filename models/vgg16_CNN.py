...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes):
        ...
        self.cnn_out = nn.Sequential(nn.Linear(1024, 256), 
                                     ...
                                     nn.Dropout(0.5))
        # 2DCNN-based temporal network
        self.temporal_net = nn.Sequential(nn.Conv2d(1, 16, kernel_size=5, stride=1, padding=2),
                                          nn.BatchNorm2d(16), 
                                          nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.cnn_out(x)
        # consider temporal info between chunks (using CNN-based model)  
        x = x.view(-1, 1, 11, x.size(1))
