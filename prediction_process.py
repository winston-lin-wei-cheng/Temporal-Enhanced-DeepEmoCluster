...
MODEL_PATH_ACT = './trained_models/TempDeepEmoCluster_epoch30_batch64_10clusters_Act_'+model_type+'.pth.tar'
MODEL_PATH_DOM = './trained_models/TempDeepEmoCluster_epoch30_batch64_10clusters_Dom_'+model_type+'.pth.tar'
MODEL_PATH_VAL = './trained_models/TempDeepEmoCluster_epoch30_batch64_10clusters_Val_'+model_type+'.pth.tar'
model_act = load_model(MODEL_PATH_ACT, num_clusters=10, model_type=model_type)
model_dom = load_model(MODEL_PATH_DOM, num_clusters=10, model_type=model_type)
model_val = load_model(MODEL_PATH_VAL, num_clusters=10, model_type=model_type)
model_act.cuda()
model_act.eval()
model_dom.cuda()
model_dom.eval()
...
for i in tqdm(range(len(paths))):
    ...
    if 'Triplet' in model_type:
        _, _, _, _, pred_act = model_act(chunk_data)
        _, _, _, _, pred_dom = model_dom(chunk_data)
        _, _, _, _, pred_val = model_val(chunk_data)        
    else:
